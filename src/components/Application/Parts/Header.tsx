import { IApplicationThemeProps, Ii18nProps, IProjectInfoProps, IUserInfoProps } from "@echino/echino.ui.sdk";
...
interface IHeaderProps extends IApplicationProps, IProjectInfoProps, Ii18nProps {
            ...
            <div className="flex grow flex-col items-center justify-between lg:flex-row lg:px-6">
                <div className="flex w-full items-center justify-between gap-2 border-b border-gray-200 px-3 py-3 dark:border-gray-800 sm:gap-4 lg:justify-normal lg:border-b-0 lg:px-0 lg:py-4">
                    ...
                    {hasLogo ?
                        <div className="flex items-center justify-center grow">
                            <NavLink to="/" className="lg:hidden">
                                {props.LogoUrl &&
                                    <img className="h-6 dark:hidden" src={props.LogoUrl} alt="Logo" />
                                }
                                {props.LogoDarkUrl &&
                                    <img className="h-6 hidden dark:block" src={props.LogoDarkUrl} alt="Logo" />
                                }
                            </NavLink>
                        </div>
                        :
                    ...
                    <Popover className="relative">
                        <Popover.Button as="button" className="z-99999 flex h-10 w-10 items-center justify-center rounded-lg text-gray-700 hover:bg-gray-100 dark:text-gray-400 dark:hover:bg-gray-800 lg:hidden">
                        ...
                        <Popover.Panel className="popover right-0 flex flex-col gap-2 p-1">
                            ...
                            <div className='flex flex-row h-10'>
                                <div className={`px-2 py-px w-10 flex items-center justify-center font-semibold cursor-pointer border rounded-l-md 
                                ${props.language.substring(0, 2) === 'fr' ? 'text-white bg-primary border-primary' : 'border-gray-300 dark:border-gray-700'}`}
                                    onClick={() => props.setLanguage('fr')}>FR</div>
                                <div className={`px-2 py-px w-10 flex items-center justify-center font-semibold cursor-pointer border-t border-b 
                                ${props.language.substring(0, 2) === 'en' ? 'text-white bg-primary border-primary' : 'border-gray-300 dark:border-gray-700'}`}
                                    onClick={() => props.setLanguage('en')}>EN</div>
                                <div className={`px-2 py-px w-10 flex items-center justify-center font-semibold cursor-pointer border-t border-b 
                                ${props.language.substring(0, 2) === 'it' ? 'text-white bg-primary border-primary' : 'border-gray-300 dark:border-gray-700'}`}
                                    onClick={() => props.setLanguage('it')}>IT</div>
                                <div className={`px-2 py-px w-10 flex items-center justify-center font-semibold cursor-pointer border rounded-r-md 
                                ${props.language.substring(0, 2) === 'de' ? 'text-white bg-primary border-primary' : 'border-gray-300 dark:border-gray-700'}`}
                                    onClick={() => props.setLanguage('de')}>DE</div>
                            </div>
                        </Popover.Panel>
                    </Popover>
                ...
                <div className="w-full items-center justify-between gap-4 px-5 py-4 shadow-theme-md lg:flex lg:justify-end lg:px-0 lg:shadow-none hidden">
                    <div className="flex items-center gap-2 2xsm:gap-3">
                        ...
                        <button
                            onClick={() => {
                                let isDark = props.applicationTheme === 'dark';
                    ...
                    <Popover className="relative">
                        <Popover.Button as="div" className="flex items-center text-gray-700 dark:text-gray-400 cursor-pointer">
                        ...
                        <Popover.Panel className="popover flex flex-col gap-2 p-1">
                            <Button Label="Logout" Type="MenuItem" OnClick={() => {
                                let appUrl = `${props._projectInfo.clusterUrl}/${props._projectInfo.serviceName}`;
                                let logoutUrl = `${appUrl}/logout?redirectUrl=${encodeURIComponent(appUrl)}`;
                                window.location.href = logoutUrl;
