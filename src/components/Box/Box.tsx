		...
		if (this.props.HasLayout) {
			...
			${this.props.Clickable ? 'cursor-pointer hover:shadow-lg hover:border-primary hover:ring-3 hover:ring-primary/10' : ''}  
				...
				${this.props.Footer ? 'dark:bg-gray-900' : 'dark:bg-white/[0.03]'}`}>
					...
					{this.props.Modal ?
						<div className={`max-h-[calc(100dvh-100px)] lg:max-h-[700px] overflow-x-hidden overflow-y-auto cn-scroll ${this.props.DisablePadding ? '' : 'p-5 md:p-6'}`}>
							{this.renderChildren()}
						</div>
						:
			...
			return (
				<div className={`@container`}>
					...
					{this.props.Modal ?
						<div className='max-h-[calc(100dvh-100px)] lg:max-h-[700px] pb-1 overflow-x-hidden px-1 overflow-y-auto cn-scroll'>
							{this.renderChildren()}
						</div> :
						this.renderChildren()
					}
